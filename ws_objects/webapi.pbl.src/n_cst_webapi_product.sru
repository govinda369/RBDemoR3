...
ls_url = ls_url + "/" + adw_data.dataobject + "/" + as_data
	...
	IF li_ret = 1 Then
		IF li_cnt = 1 And ldws_substatus = NewModified! Then
			...
			If gi_service_type = 3 Then
				adw_browser.ImportRowFromJson(ls_json, li_subcaterow + 1)	
			Else
				adw_browser.ImportJsonByKey(ls_json)
			End If
			adw_browser.event rowfocuschanged(li_subcaterow)
	...
	Choose Case li_cnt
		Case 1
			...
			IF li_ret = 1 And ldws_productstatus = NewModified! Then
				...
				If gi_service_type = 3 Then
					adw_master.ImportRowFromJson(ls_json, li_productrow + 1)
				Else
					adw_master.ImportJsonByKey(ls_json)
				End If
			End IF
